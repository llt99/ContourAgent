...
# from data_query import text_to_sql_query  # 原始数据库版本
from data_query_test import text_to_sql_query  # 测试数据版本
...
def text_to_sql_query_tool(query: str):
...
async def kriging_interpolate(
    ...
    **kwargs
    ...
    final_params = {"method": method, "variogram_model": final_model, "autoOptimizeModel": autoOptimizeModel, "drift": drift}

    # ------------------- 安全插值 -------------------
    def safe_interpolate(lons, lats, values, params):
        try:
            res = interpolator.interpolate(lons, lats, values, params)
    ...
    final_interp = await asyncio.to_thread(safe_interpolate, lons, lats, values, final_params)
...
async def render_map_tool(ctx, grid_x, grid_y, z, points=None, boundary_geom=None,
    ...
    result = await renderer.render_map(
        grid_x, grid_y, z, points, boundary_geom,
