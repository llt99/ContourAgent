...
SYSTEM_PROMPT = """
    ...
    - method: 插值方法，如果用户未指定，请填 null 表示待自动选择
    - model: 半变异函数模型，如果用户未指定，请填 null 表示待自动选择
    - method_code: 插值方法代码，如果未指定填 null
    - model_code: 变异函数模型代码，如果未指定填 null
    - variable_code: 若有对应编码，否则为 null
    - plot_code: 若有对应编码，否则为 null
    ...
    - 可用的 Agent 包括: "nlp", "data", "kriging", "image", "feedback"。
    - 根据用户意图决定需要哪些 Agent。例如，如果用户只想查询数据，pipeline 可能只有 ["nlp", "data"]。
    - 如果是标准的绘图任务，pipeline 通常是 ["nlp", "data", "kriging", "image"]。
    ...
    - 输出 JSON 示例结构如下：
    {
      ...
      "plan": {
        "pipeline": ["nlp", "data", "kriging", "image"]
      }
    }
"""
